...
object ImageManager {
    ...
    private fun loadImage(context: Context?, url: String?, file: File?, resId: Int, errImg: Int, emptyImg: Int, iv: ImageView) {
        ...
        if (Util.isOnMainThread() && context != null) {
            if (context is Activity)if (CheckUtils.checkActivityIsRunning(context)) return
            ...
            when {
                ...
                file != null -> Glide.with(context).load(file).placeholder(empty).error(err).transition(withCrossFade()).into(iv)
                resId != -1 -> Glide.with(context).load(resId).placeholder(empty).error(err).transition(withCrossFade()).into(iv)
            }
        }
    }
    ...
    private fun loadCircleImage(context: Context?, url: String?, file: File?, resId: Int, errImg: Int, emptyImg: Int, iv: ImageView) {
        ...
        if (Util.isOnMainThread() && context != null) {
            ...
            if (empty == -1) empty = defaultEmpty
            when {
                !TextUtils.isEmpty(url) -> Glide.with(context).load(url).centerCrop().placeholder(empty).error(err).transform(CircleCrop()).transition(withCrossFade()).into(iv)
                file != null -> Glide.with(context).load(file).centerCrop().placeholder(empty).error(err).transform(CircleCrop()).transition(withCrossFade()).into(iv)
                resId != -1 -> Glide.with(context).load(resId).transform(CircleCrop()).centerCrop().placeholder(empty).error(err).transition(withCrossFade()).into(iv)
            }
        }
    }
    ...
    private fun loadRoundImage(context: Context?, url: String?, file: File?, resId: Int, radius: Int, errImg: Int, emptyImg: Int, iv: ImageView) {
        ...
        if (Util.isOnMainThread() && context != null) {
            ...
            when {
                !TextUtils.isEmpty(url) ->
                    Glide.with(context).load(url).transform(CenterCrop(),RoundedCorners(radius)).placeholder(empty).error(err).transition(withCrossFade()).into(iv)
                file != null ->
                    Glide.with(context).load(file).transform(CenterCrop(),RoundedCorners(radius)).placeholder(empty).error(err).transition(withCrossFade()).into(iv)
                resId != -1 ->
                    Glide.with(context).load(resId).transform(CenterCrop(),RoundedCorners(radius)).placeholder(empty).error(err).transition(withCrossFade()).into(iv)
            }
        }
    }
