 ...
 */
    ...
    private fun initDialog() {
        ...
        selectorDialog = QuickDialog.Builder(context, R.layout.app_dialog_selector_time)
    ...
    class Builder(var context: Context, var listener: (date: Date, timestamp: Long, dateStr: String) -> Unit) {
        ...
        fun build(): SelectorTimeDialog {
            ...
            if (TextUtils.isEmpty(startTime))
                startTime = DateUtils.toStr(DateUtils.timeInMillis(),DateUtils.YMDHM)
            if (TextUtils.isEmpty(endTime))
                endTime = DateUtils.beforeDayToStr(1)
            if (mode == null)
                mode = MODE.YMDHM
