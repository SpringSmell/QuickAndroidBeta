...
import org.quick.utils.CommonUtils
...
abstract class BannerFragment<M> : ThemeFragment() {
        ...
        onPageChangeListener = object : ViewPager.OnPageChangeListener {
            ...
            override fun onPageSelected(position: Int) {
                ...
                if (isShowHintDots && adapter!!.count > 1) {
                    ...
                    dotsContainer.getChildAt(tempPosition)
                        .setBackgroundResource(onResultIndicatorDotBgFocus())
...
//        }
    ...
    private val lastSelectItemPosition: Int
        get() {
            ...
            if (isAutoScroll) {
                when {
                    tempPosition >= adapter!!.count - 1 -> tempPosition =
                        Integer.MAX_VALUE / 2 - Integer.MAX_VALUE /
                                2 % dataList.size
                    tempPosition == 0 -> tempPosition =
                ...
                if (isAutoScroll)
                    when (event.action) {
                        MotionEvent.ACTION_DOWN -> {
                            ...
                            setScrollTimeType(onResultScrollChangeTimeNormal())
                            stopAutoScroll()
                            if (isResolveSwipeRefreshLayout && Math.abs(event.x - downPoint.x) < Math.abs(
                                    event.y - downPoint.y
                                )
                            )
                            //Y轴距离大于X轴
    ...
    open fun onResultIndicatorDot(): View {
        ...
        val size = FormatUtils.dip2px(activity!!, onResultIndicatorDotSize().toFloat()).toInt()
        val margin = FormatUtils.dip2px(activity!!, onResultIndicatorDotMargin().toFloat()).toInt()
    ...
    abstract fun onBindData(position: Int, holder: ViewHolder, itemData: M)


    fun setResolveSwipeRefreshLayout(
